defmodule Exduckdb.DuckDB do
  ...
  def close(conn), do: DuckDBNIF.close(conn)
  ...
  **Note**: If triggers are used, the count may be larger than expected.
  ...
  def last_insert_rowid(conn), do: DuckDBNIF.last_insert_rowid(conn)

  @spec transaction_status(db()) :: {:ok, :idle | :transaction}
  def transaction_status(conn), do: DuckDBNIF.transaction_status(conn)
  ...
  def serialize(conn, database \\ "main") do
    DuckDBNIF.serialize(conn, String.to_charlist(database))
  end
  ...
  def deserialize(conn, database \\ "main", serialized) do
    DuckDBNIF.deserialize(conn, String.to_charlist(database), serialized)
  end
  ...
  def release(conn, statement) do
    DuckDBNIF.release(conn, statement)
  end
